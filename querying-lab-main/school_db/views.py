...
def problem_one(request):
  a_and_b_students = Student.objects.filter(gpa__gt=3).order_by('-gpa')
  for student in a_and_b_students:
...
def problem_two(request):
  pre_2010_instructor_hires = Instructor.objects.filter(hire_date__year__lt = 2010).order_by("hire_date")
  for instructor in pre_2010_instructor_hires:
...
def problem_three(request):
  instructor_pk_2 = Instructor.objects.get(pk=2)
  instructor_pk_2_courses = Course.objects.filter(instructor = 2)
  print(f"Instructor Name: {instructor_pk_2.first_name} {instructor_pk_2.last_name}")
  print("Courses:")
  for course in instructor_pk_2_courses:
    print(f"  - {course.name}")
...
def problem_four(request):
  ...
  number_of_instructors = Instructor.objects.all().count()
  print(f"Students Count: {number_of_students}")
...
def problem_five(request):
  myself = Student.objects.create(first_name="Amy", last_name="Dworsky",year= 2022, gpa=3.0)
  print(f"ID: {myself.id}")
  print(f"Full Name: {myself.first_name} {myself.last_name}")
  print(f"Year: {myself.year}")
  print(f"GPA: {myself.gpa}")
...
def problem_six(request):
  ...
  student_id = 13
  Student.objects.filter(id=student_id).update(gpa=3.2)
  student_pk_11 = Student.objects.get(id=student_id)
  print(f"ID: {student_pk_11.id}")
  print(f"Full Name: {student_pk_11.first_name} {student_pk_11.last_name}")
  print(f"GPA: {student_pk_11.gpa}")
